...
{
    public sealed class ApplyCouponCommandHandler : ICommandHandler<ApplyCouponCommand, bool>
    {
        ...
        public ApplyCouponCommandHandler(ICartRepository repo, IEventStore store, IOutboxPublisher outbox, ISnapshotPolicy snapshotPolicy, ISnapshotWriter snapshotWriter)
        {
            _repo = repo; _store = store; _outbox = outbox; _snapshotPolicy = snapshotPolicy; _snapshotWriter = snapshotWriter;
        }

        public async Task<bool> Handle(ApplyCouponCommand command, CancellationToken ct)
        {
            await _repo.ApplyCouponAsync(command.CartId, command.Code, command.Amount, command.Description, ct);
            var evt = new CouponAppliedV1(command.CartId, command.Code, command.Amount, command.Description, 0, DateTime.UtcNow, "CartService");
            var version = await _store.AppendAsync(command.CartId, new[] { evt }, "CartService", ct);

            await _outbox.EnqueueAsync("Cart.CouponApplied.v1", evt, "promotion", ct);
