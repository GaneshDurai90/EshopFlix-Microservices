...
{
    public sealed class UpdateQuantityCommandHandler : ICommandHandler<UpdateQuantityCommand, int>
    {
        ...
        public UpdateQuantityCommandHandler(ICartRepository repo, IEventStore store, IOutboxPublisher outbox, ISnapshotPolicy snapshotPolicy, ISnapshotWriter snapshotWriter)
        {
            _repo = repo; _store = store; _outbox = outbox; _snapshotPolicy = snapshotPolicy; _snapshotWriter = snapshotWriter;
        }

        public async Task<int> Handle(UpdateQuantityCommand command, CancellationToken ct)
        {
            var affected = await _repo.UpdateQuantity(command.CartId, command.ItemId, command.DeltaQuantity);
            var evt = new ItemQuantityUpdatedV1(command.CartId, command.ItemId, command.DeltaQuantity, 0, DateTime.UtcNow, "CartService");
            var version = await _store.AppendAsync(command.CartId, new[] { evt }, "CartService", ct);

            await _outbox.EnqueueAsync("Cart.ItemQuantityUpdated.v1", evt, "inventory", ct);
